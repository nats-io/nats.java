...
import static io.nats.client.support.URIUtils.withPort;
...
public class RunProxy implements Runnable {
     ...
     */
    public RunProxy(URI hostPort, SSLContext sslContext, ExecutorService executor) throws IOException {
        if (null == hostPort.getHost()) {
        ...
        if (null != sslContext) {
            server = sslContext.getServerSocketFactory().createServerSocket(
                port,
                10,
                InetAddress.getByName(hostPort.getHost()));
        } else {
        ...
        hostPort = withPort(hostPort, server.getLocalPort());
