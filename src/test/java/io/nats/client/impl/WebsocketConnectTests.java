...
import static io.nats.client.support.URIUtils.withHost;
...
public class WebsocketConnectTests {
    ...
    public void testProxy() throws Exception {
        ...
        RunProxy proxy = new RunProxy(new URI("//localhost:0"), null, executor);
        executor.submit(proxy);

        try (NatsTestServer ts = new NatsTestServer("src/test/resources/ws.conf", false)) {
            Options options = new Options.Builder()
                .server(withHost(new URI(ts.getURI("ws")), "127.0.0.1") + "")
                .maxReconnects(0)
                .proxyURI(new URI("http://localhost:" + proxy.getPort()))
                .build();
            try (Connection connection = standardConnection(options)) {
                Dispatcher dispatcher = connection.createDispatcher(msg -> {
                    connection.publish(msg.getReplyTo(), (new String(msg.getData()) + ":REPLY").getBytes());
    ...
    // FIXME: I think I need to generate a server key for this to work.
    /*
    @Test
    public void testSecureProxy() throws Exception {
        SSLContext ctx = TestSSLUtils.createTestSSLContext();
        ExecutorService executor = Executors.newFixedThreadPool(3);
        RunProxy proxy = new RunProxy(new URI("//localhost:0"), ctx, executor);
        executor.submit(proxy);
        ...
        try (NatsTestServer ts = new NatsTestServer("src/test/resources/ws.conf", false)) {
            Options options = new Options.Builder()
                ...
                .sslContext(ctx)
                .maxReconnects(0)
                .proxyURI(new URI("https://localhost:" + proxy.getPort()))
                .build();
            try (Connection connection = standardConnection(options)) {
                Dispatcher dispatcher = connection.createDispatcher(msg -> {
                    connection.publish(msg.getReplyTo(), (new String(msg.getData()) + ":REPLY").getBytes());
