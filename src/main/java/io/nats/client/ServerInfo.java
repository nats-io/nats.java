...
class ServerInfo {
	...
	public String toString() {
		String rv = String.format("INFO {\"server_id\":\"%s\",\"version\":\"%s\",\"go\":\"%s\","
				+ "\"host\":\"%s\",\"port\":%d,\"auth_required\":%b,\"ssl_required\":%b,"
				+ "\"tls_required\":%b,\"tls_verify\":%b,\"max_payload\":%d}\r\n",
				this.id, this.version, this.goVersion, this.host, this.port, 
				this.authRequired, this.tlsRequired, this.tlsRequired, this.tlsVerify,
				this.maxPayload );
		return rv;
	}
}