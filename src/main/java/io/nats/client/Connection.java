...
public interface Connection<T extends Message> extends AutoCloseable {
     ...
     */
    CompletableFuture<Message> request(String subject, byte[] data);

    /**
     * Send a request. The returned future will be completed when the
     * response comes back.
     *
     * @param requestMessage message that you are sending
     * @return a Future for the response, which may be cancelled on error or timed out
     */
    CompletableFuture<Message> request(final Message requestMessage);
     ...
     * the timeout and handling the ExecutionException and TimeoutException.
     *
     * @param requestMessage request message that you are sending
     * @param timeout the time to wait for a response
     * @return the reply message or null if the timeout is reached
     * @throws InterruptedException if one is thrown while waiting, in order to propogate it up
     */
    Message request(final Message requestMessage, final Duration timeout) throws InterruptedException;
