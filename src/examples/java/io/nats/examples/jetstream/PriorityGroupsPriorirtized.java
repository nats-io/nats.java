...
/**
 * This example demonstrates a prioritized consumer for priority groups.
 *
 ...
 *
 * This example shows a prioritized standby worker activating when the primary worker
 * slows down.
 *
 */
public class PriorityGroupsPriorirtized {
	 ...
	 private static final int PRE_FETCH_SIZE = 2;
	 ...
	 public static void primaryConsumer() {
	     ...
	     try (Connection nc = Nats.connect(options)) {
	         ...
	         MessageHandler handler = msg -> {
	             msg.ack();
	             //Slow down a bit after each message
	             sleepTime +=100;
	 ...
	 public static void standbyConsumer() {
	     ...
	     try (Connection nc = Nats.connect(options)) {
	         ...
	         System.out.println("Starting standby consumer - Will receive messages once the primary slows down enough.");
	 ...
	 public static void main(String[] args) {
	     ...
	     try (Connection nc = Nats.connect(options)) {
	         ...
	         streamContext.createOrUpdateConsumer(ConsumerConfiguration.builder()
	        		 ...
	        		 .priorityPolicy(PriorityPolicy.Prioritized)  //Prioritized policy
	        		 .maxAckPending(20)
	        		 .build());
